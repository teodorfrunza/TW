<html>
    ...
    <body content="width=device-width, initial-scale=1.0">
        ...
        <?php
			...
			$stid2 = oci_parse($connection, 'SELECT COUNT(*) FROM DETINUTI');
			if (!$stid2) {
				$e = oci_error($conn);
			...
			$stid = oci_parse($connection, 'SELECT * FROM (SELECT * FROM DETINUTI WHERE ROWNUM<:startrow+30 MINUS SELECT * FROM DETINUTI WHERE ROWNUM<:Startrow) WHERE ROWNUM<30');
